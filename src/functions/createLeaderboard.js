...
module.exports = async (client, interaction, title, lb, ephemeral = false) => {
  let page = 1;
  const replyOptions = { embeds: [generateEmbed(0, lb, title, page)] };
  ...
  let message = await interaction.editReply({ embeds: [generateEmbed(0, lb, title, page)], components: [row] });
  let currentIndex = 0;
  ...
  collector.on('collect', async (btn) => {
    ...
    // Defer the button interaction FIRST to avoid Unknown Interaction error
